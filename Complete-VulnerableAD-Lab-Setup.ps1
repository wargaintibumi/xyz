...
Network Mode     : $(if ($UseDHCP) { "DHCP (Bridged)" } elseif ($SkipNetworkConfig) { "Manual/Existing" } elseif ($StaticIP) { "Static IP: $StaticIP" } else { "DHCP (Bridged)" })
...
function Set-NetworkConfiguration {
    if ($SkipNetworkConfig) {
        Write-Warning "Skipping network configuration (SkipNetworkConfig flag set)"
        $Global:SetupState.NetworkConfigured = $true
        $Global:SetupState.LastStep = "Network Skipped"
        Save-SetupState
        return $true
    }
    ...
    try {
        ...
        Write-Status "Interface Description: $($adapter.InterfaceDescription)"
        
        # Check if using DHCP or Static
        if ($UseDHCP -or [string]::IsNullOrEmpty($StaticIP)) {
            Write-Status "Configuring network for DHCP (Bridged mode)..."
            
            # Enable DHCP
            Set-NetIPInterface -InterfaceAlias $adapter.Name -Dhcp Enabled -ErrorAction Stop
            
            # Set DNS to get from DHCP initially, will change to loopback later
            ...
            if ($ipConfig) {
                ...
                Write-Warning "After DC promotion, DNS will be set to 127.0.0.1"
                Write-Status "Make sure your router/DHCP doesn't change this IP!"
            } else {
                Write-Warning "No IP address assigned yet, continuing anyway..."
            }
            
        } else {
            Write-Status "Configuring static IP address: $StaticIP"
            
            if ([string]::IsNullOrEmpty($DefaultGateway)) {
                Write-Failure "Static IP specified but no gateway provided"
                Write-Status "Use: -StaticIP '192.168.1.10' -DefaultGateway '192.168.1.1'"
                return $false
            }
        ...
        # Set DNS to loopback (this server will be the DNS server after DC promotion)
        Write-Status "Setting DNS server to 127.0.0.1 (localhost)"
        Set-DnsClientServerAddress -InterfaceAlias $adapter.Name -ServerAddresses "127.0.0.1" -ErrorAction Stop
        
        # Disable IPv6 (optional, reduces noise in lab environment)
        ...
        Write-Warning "You may need to configure network manually"
        Write-Status "Current network settings:"
